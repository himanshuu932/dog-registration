...
const textContent = { //
  en: { //
    ...
    save: 'Save', //
    cancel: 'Cancel', //
    nameLabel: 'Username', // Changed label from 'Name' to 'Username' for clarity
    email: 'Email', //
    role: 'Role', //
    phone: 'Phone', //
  ...
  hi: { //
    ...
    save: 'सहेजें', //
    cancel: 'रद्द करें', //
    nameLabel: 'उपयोगकर्ता नाम', // Changed label from 'नाम' to 'उपयोगकर्ता नाम'
    email: 'ईमेल', //
    role: 'भूमिका', //
    phone: 'फ़ोन', //
...
function Navbar({ languageType = 'en', user, notifications = [], onLogout, setLanguageType, onProfileUpdate }) {
  const [showMenu, setShowMenu] = useState(false); //
  const [showNotif, setShowNotif] = useState(false); //
  const [showProfilePopover, setShowProfilePopover] = useState(false); //
  const [isEditingProfile, setIsEditingProfile] = useState(false); //
  // Use editableUsername now
  ...
  const handleSaveProfile = async () => { //
    ...
    try {
      ...
      const response = await fetch(`${backend}/api/auth/profile`, { //
        ...
        body: JSON.stringify({ username: editableUsername }), // Send 'username'
      });
              ...
              {user && ( //
                <div className="navbar__user-section" ref={userDisplayRef}> {/* */}
                  <button className="navbar__username-btn" onClick={toggleProfilePopover}> {/* */}
                    ...
                    {user.username} {/* Display user.username directly */}
                  </button>
                  {showProfilePopover && ( //
                      ...
                      {!isEditingProfile ? ( //
                        <>
                          <div className="profile-details"> {/* */}
                            {/* Use currentText.nameLabel for the label */}
                            <p><strong>{currentText.nameLabel}:</strong> {user.username}</p> {/* Display user.username */}
                        ...
                        <div className="profile-edit-form"> {/* */}
                          <div className="form-group"> {/* */}
                            ...
                            <input
                              type="text"
                              id="profileUsername" // Changed id
                              value={editableUsername} // Use editableUsername
                              onChange={(e) => setEditableUsername(e.target.value)} // Use setEditableUsername
                            />
                          </div>
                          <p><strong>{currentText.email}:</strong> {user.email || 'N/A'} <small>{currentText.fieldCannotBeChanged}</small></p> {/* */}
                          <p><strong>{currentText.phone}:</strong> {user.phone || 'N/A'} <small>{currentText.fieldCannotBeChanged}</small></p> {/* */}
            ...
            {user && ( //
              <li className="nav-item nav-item--gov mobile-user-profile-section"> {/* */}
                <div className="mobile-user-info" ref={userDisplayRef} onClick={toggleProfilePopover}> {/* */}
                    <UserCircle size={20} /> {user.username} {/* Display user.username */}
                ...
                {showProfilePopover && showMenu && ( //
                     <div className="profile-popover profile-popover--mobile" ref={profilePopoverRef}> {/* */}
                        ...
                        <div className="profile-edit-form"> {/* */}
                            ...
                            <input
                                type="text"
                                id="profileUsernameMobile" // Changed id
                                value={editableUsername} // Use editableUsername
                                onChange={(e) => setEditableUsername(e.target.value)} // Use setEditableUsername
                            />
                            </div>
